...
those nested under `META_MERGE`) are parsed, as are legacy top-level
`recommends => { ... }` and `suggests => { ... }` blocks (mapped
to runtime `recommends` / `suggests`).  Inline comments attached to
