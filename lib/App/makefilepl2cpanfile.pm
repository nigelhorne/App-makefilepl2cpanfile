...
sub generate {
    ...
    # Develop deps from config or defaults
    if ($with_dev) {
        my %default = (
            'Perl::Critic'        => 0,
            'Devel::Cover'        => 0,
        ...
        my $cfg = File::HomeDir->my_home . '/.config/makefilepl2cpanfile.yml';
        if (-r $cfg) {
            my $y = YAML::Tiny->read($cfg)->[0];
            %default = %{ $y->{develop} } if $y->{develop};
        }

        for my $m (keys %default) {
            $deps{develop}{$m} //= $default{$m};
        }
    }
